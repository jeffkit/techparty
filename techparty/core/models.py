...
class UserProfile(models.Model):
    ...
    avatar = models.URLField(u'头像', null=True, blank=True)
    custom_avatar = models.ImageField(u'自定义头像', upload_to='avatars',
...
class Event(models.Model):
    ...
    hash_tag = models.CharField(u'微博标签', max_length=20,
                                blank=True, null=True)  # 用于微博、Twitter
...
class Enroll(models.Model):
    ...
    comment = models.CharField(u'报名理由', max_length=140)
...
class Topic(models.Model):
    ...
    event = models.ForeignKey(Event, verbose_name=u'活动')
    title = models.CharField(u'标题', max_length=50)
    sub_title = models.CharField(u'副标题', max_length=50,
                                 null=True, blank=True)
...
class Photo(ImageModel):
    ...
    original_image = models.ImageField(u'原图',
                                       upload_to='photos/%Y/%m/%d')
    num_views = models.PositiveIntegerField(u'查看次数',
...
class LiveMessage(models.Model):
    ...
    create_by = models.ForeignKey(User, verbose_name=u'作者',
                                  related_name="createdMessage")
    create_time = models.DateTimeField(u'发送时间', auto_now_add=True)
    update_by = models.ForeignKey(User, verbose_name=u'最后修改人',
                                  related_name="updatedMessage")
...
def create_user_profile(sender, instance, created, **kwargs):
    ...
    if not get_profile():
        profile = UserProfile(user=instance, description=' ',
                              is_lecturer=False)
        profile.save()
...
def facebook_extra_values(sender, user, response, details, **kwargs):
    ...
    profile.avatar = 'http://graph.facebook.com/%s/picture?type=large'\
                     % response.get('id')
    profile.save()
    return True
